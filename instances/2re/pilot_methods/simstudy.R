...
setwd("instances/2re/pilot_methods/")
...
write_simstudy_iridis(setup, subcases, run_simstudy_each,
                      time_each = 20, iridis_dir = "main_run")
...
#' the bayesFPCA model choice part is very slow: don't run for everything
model_choice_methods <- c("bayesFPCA-model-choice-L15")

model_choice_setup <- expand_grid(
    case = "2re",
    subcase_id = 1,
    method = model_choice_methods,
    seed = 1:2
)

write_simstudy_iridis(model_choice_setup, subcases, run_simstudy_each,
                      time_each = 120, mem = 8, iridis_dir = "model_choice_run")

mc_simstudy <- read_simstudy_iridis("model_choice_run")

mc_status <- make_complete_run_status(
    model_choice_setup,
    mc_simstudy
)
